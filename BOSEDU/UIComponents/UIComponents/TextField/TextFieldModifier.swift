...
public struct TextFieldModifier: ViewModifier {
    ...
    private var foregroundColor: Color {
        if error {
            return Color.accent60
        } else {
            return isFocused ? Color.primary70 : Color.neutral70
        }
    }
    ...
    public func body(content: Content) -> some View {
        content
            ...
            .foregroundStyle(style == .defaultStyle ? Color.neutral70 : Color.neutral100)
            .robotoFont(size: style == .defaultStyle ? 14 : 20, font: .semiBold)
            .multilineTextAlignment(style == .defaultStyle ? .leading : .center)
            .background(error ? Color.accent20.opacity(0.5) : .clear)
